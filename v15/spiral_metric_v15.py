...
#
"""
spiral_metric_v15.py
====================
指标层 (体检报告) —— 与 spiral_model_v15.py 配对, 由它 import。
...
【文件结构 (按出现顺序)】
  ...
  8 编排与作图         collect_metrics_v15 (总入口) / plot_metrics_v15 (20 面板)
...
【被谁 import】 spiral_model_v15.py: collect_metrics_v15 / plot_metrics_v15 /
              metrics_for_json
"""
import os
import json
...
def exact_ground_state(L, J=1.0, h=1.0):
    ...
    spiral_model_v15.exact_ground_state 在 L=18 上实测建 H 1.53s + eigsh 3.86s
...
# 相同的参数与单位**, 只变网格分辨率 —— 见 spiral_v15_prepare.py 的文件头。
...
def tier3_structural_ladder(s6, data_dir, clip_seeds=(0, 5, 10),
    ...
    if missing:
        ...
        record_metric(
            ...
            '一层同单位受控参考侧, 由 spiral_v15_prepare.py 生成。',
            ...
            note='这是不可计算, 不是"不达标"。**本仓库里可运行的生成脚本只有两个**: '
                 'spiral_v14_prepare.py (自建参考侧) 与 spiral_v15_prepare.py '
    ...
    print(f"    D_struct = {_lda:.4f}   种子散布 = {_lsc:.4f} ({len(_lpair)} 对)   "
          f"比值 = {_lratio:.3f}")
    print(f"    复核 spiral_metric_v15.V13_A_BASELINE: "
    ...
    if _v15bad:
        print(f"\n  【v15·W2 同单位受控】缓存缺失或不可用 -> 本小节跳过 "
              f"(不影响上面的 A1/A2)")
        print(f"      生成: python spiral_v15_prepare.py  ->  {v15file}")
        ...
        record_metric(
            ...
            '(spiral_v15_prepare.py, N=36/72/144, dt 随 h^2 同比缩), 于是两侧是'
    ...
    record_guard(
        ...
        note='**这是缺口的原因之一被查清, 不是一条失败的检验** (诊断项, 不计入'
             ...
             '更新式已逐字核对 spiral_model_v15.stage6_life。'
    ...
    out['_plot'] = {
        'A': {'keys': list(_KEYS), 'seeds': seeds,
              ...
              # result/_v15_data.json 的 A1.ref_panel 里。
...
def metric_hj_negative_control(hj):
    ...
    详见 spiral_model_v15.hj_scan 的函数头):
    ...
    if not hj:
        record_metric(2, '5 临界性负对照 (h/J 扫描)', 'argmax c_fit @ h/J=1',
                      ...
                      'spiral_model_v15.hj_scan()',
                      '横场 Ising 热力学临界点 h/J=1', None, None,
                      '未运行 (ctx 无 hj)', None,
                      note='负对照未执行, 无法判定。')
        record_guard('F2 负对照已运行', 'collect_metrics_v15',
                     'ctx["hj"] 存在', False)
        return None
    ...
    record_metric(
        ...
        'spiral_model_v15.hj_scan(): 逐点精确对角化 + Calabrese-Cardy 拟合',
    ...
    **分层标注** (审计 §三之补 F1① 的降级要求, 必须写在指标名里):
    ...
    if not f1:
        record_metric(2, '6 跨边界条件中心荷交叉检验 (同族)',
                      '|c_twist - c_PBC| / c_PBC',
                      '换边界条件后中心荷读数是否稳定', 'spiral_model_v15'
                      '.cross_boundary_twist()',
                      '临界 Ising c=0.5', None, None,
                      '未运行 (ctx 无 f1)', None,
                      note='未执行, 无法判定。')
        record_guard('F1 跨边界条件已运行', 'collect_metrics_v15',
                     'ctx["f1"] 存在', False)
        return None
    ...
    record_metric(
        ...
        'spiral_model_v15.cross_boundary_twist(): 精确对角化 + 检查点选择协议 MERA '
...
def metric_consistency_checks(f5):
    ...
    if not f5:
        record_metric(2, '7 一致性检查束 (数值路径 <-> 解析路径)', '5 条各自的双路径偏差',
                      '实现与它声称的解析式是否一致', 'spiral_model_v15'
                      '.consistency_checks()',
                      'JW 闭合式 / 幂迭代残差 / Forman 恒等式 / '
                      '离散 Laplacian 零和 / 平衡恒等式 / 熵上界',
                      None, None, '未运行 (ctx 无 f5)', None,
                      note='未执行, 无法判定。')
        record_guard('F5 一致性检查已运行', 'collect_metrics_v15',
                     'ctx["f5"] 存在', False)
        return None
    ...
    record_metric(
        ...
        'spiral_model_v15.consistency_checks(): eigsh vs JW 闭合式 / 幂迭代残差 '
    ...
    try:
        with open(os.path.join(_mod_dir, 'spiral_model_v15.py'),
                  encoding='utf-8') as _f:
            _eq_lines = [l for l in _f.read().splitlines() if 'equal_state' in l]
    ...
    record_guard('W11 诊断: L1 有到下游的因果通路 (负对照的前提)',
                 'stage1_void',
                 f"spiral_model_v15.py 中 equal_state 出现 {len(_eq_lines)} 处, "
                 ...
                 note='**诊断项, 不计入通过率 (本项设计上就该不通过)**: 本条把'
                      ...
                      '(2) **实测**: stage1_void 返回 {unitary, equal_state, '
                      'dimension}。整仓扫描 (v14/v15 两版) —— equal_state '
                      '**从未被取用**; unitary 只取其条件数上报 '
                      '(spiral_model_v15.py:3631); dimension 只喂 '
                      'derive_L1_local_to_full (:3396), 而后者算出的 dim_full '
                      '**只被 print** (:3397-3398), 从不与链的实际维数 '
                      '2**L_chain 比对。'
                      '(3) **所以**: L1 到下游**没有因果通路**。计划里那个形式'
                      '造不出来 —— 这是本条与"回头再做"的区别。'
                      '**边界**: 本条只否证那一个形式, 不给替代。**L1 的负对照'
                      '仍然是 0 条**; 若要做, 得先给 L1 建一条真实的下游连线, '
                      '那是改模型, 不在本轮工作包内。')
...
def metric_selfref_N_dependence(nd):
    ...
    if not nd:
        record_metric(2, '9 自指变体秩1化的 N 依赖 (相图)', '—', '—',
                      'spiral_model_v15.selfref_N_dependence()', None, None,
    ...
    record_metric(
        ...
        ' A 的秩1方向是不动点还是暂态?',
        'spiral_model_v15.selfref_N_dependence(): 逐 N 调 selfref_coupled, '
...
def metric_spectral_central_charge(csp):
    ...
    if not csp:
        record_metric(2, '10 谱学中心荷 (能谱族)', '—', '—',
                      'spiral_model_v15.spectral_central_charge()', None, None,
    ...
    record_metric(
        ...
        'spiral_model_v15.spectral_central_charge(): 稀疏 eigsh 取 H 头三个'
    ...
    record_guard('W3b v 无关比值单调趋 8', 'spectral_central_charge',
                 ...
                 ratio_ok,
                 note='比值只含 Delta2/Delta1, 不含 c 也不含 v, 所以它是**独立佐证**'
                      '而非重述: 它证明 Delta1=1/8 那个前提本身站得住。')
    ...
    **本条的核心是一条负面结果, 照实登记**: P0 按字面做不出来。在 L<=16 上
    ...
    if not alw:
        record_metric(2, '11 面积律 vs 对数律', '—', '—',
                      'spiral_model_v15.area_vs_log_law()', None, None,
    ...
    record_metric(
        ...
        'spiral_model_v15.area_vs_log_law(): 逐 h/J 逐 L 求基态并算 S(L/2), '
    ...
    **本条最重要的产出是一条方法学事实, 不是一个物理数**: 对 c / E0/L / xi / gap
    ...
    if not fss:
        record_metric(2, '12 有限尺寸标度', '—', '—',
                      'spiral_model_v15.finite_size_scaling()', None, None,
    ...
    record_metric(
        ...
        'spiral_model_v15.finite_size_scaling(): 逐 L 单独求基态并算全部读数'
...
_NEG_CTRL_TABLE = (
    ('L1', '空无基底等权叠加蕴潜能', None, None, (), '缺',
     '本层在 main() 里由 stage1_void(dimension=2) 一步给出: 等权叠加 + 归一。'
     '登记在册的守卫里没有任何一条 where/条件指向它 —— G1 是 L2 的, '
     'F5 束从 L2 起编。**缺口如实留着**, 不用"等权叠加显然成立"来填。'),
...
def metric_negative_control_ledger():
    ...
    两条判定分开:
      ...
      (b) **诊断** (expect_pass=False): 覆盖了几个层。**故意不计分** ——
          ...
          待办清单给自己打分。实测 L1/L4 无负对照、L7 仅一条上界, 这个缺口
          是本次的**真实产出**, 不该被一个百分比掩掉。
    """
    ...
    record_metric(
        ...
        '答审计表 P2「负对照系统化 (逐层)」',
        'spiral_metric_v15._NEG_CTRL_TABLE 与 GUARDS 注册表对表; '
        ...
        note='**负对照的判据比"有个检查"严**: 它必须能在结论不成立时失败。'
             ...
             'L1 仍**没有负对照** —— W11 查清了原因: L1 到下游没有因果通路 '
             '("推离初态"这条控制形式造不出来), 该缺口**如实留着**。')
...
def metric_geometry_robustness(s5):
    ...
    if not s5 or not s5.get('controls'):
        record_metric(2, '8 涌现几何的对照鲁棒性', 'MERA 在零模型对照分布中的百分位',
                      ...
                      'spiral_model_v15.geometry_controls()', '三类零模型对照族',
                      None, None, '未运行 (ctx 无对照族)', None,
                      note='未执行, 无法判定。')
        record_guard('F6 对照几何族已运行', 'collect_metrics_v15',
    ...
    record_metric(
        ...
        'spiral_model_v15.geometry_controls(): 三类零模型 x 3 实例, '
    ...
    # 实测换 seed 重建, 边集逐位相同 (见 spiral_model_v15 阶段五)。登记它是
    ...
    if _bi:
        ...
        record_guard(
            ...
            note='**诊断项, 不计入通过率 (本项设计上就该不通过)**: 本项问的是'
                 ...
                 '(4) **代码锚点 + 解析性质**: spiral_model_v15.build_mera_graph '
...
def collect_metrics_v15(ctx, KNOBS, _HERE, v13):
    """
    体检报告编排。
    ...
    # 新增守卫: 结论的稳健性前提
    record_guard('G6 最坏轨迹也达标', 'collect_metrics_v15',
...
# 指标层的 20 面板图
# ===========================================================================
def plot_metrics_v15(metrics, s2, s3a, out_png):
    """
    ...
    # 描述子在 result/_v15_data.json 的 A1 里。
