...
%  is established. Every number is transcribed from:
%    * _v15_run.log              (frozen full run, 1245 s; guards/metrics/ledger;
%                                 byte-identical to _v15_run_frozen.out. The earlier
%                                 1138 s log it replaced is no longer on disk.)
%    * _v15_anchor_external.py   (read-only json; external anchor, run in the A0 pass)
%    * _v15_smoke_b1_chi.py      (this pass; chi/seed invariance of the MERA bulk graph)
%  Diagnostics that "fail by design" are registered as such and are never
%  re-scored to make them pass.biblatex
%
%  NOTE: this source has NOT been compile-tested (no TeX distribution was
%  available at authoring time). Treat the first pdflatex run as unverified.
% ============================================================================
...
\title{\bfseries A per-layer negative-control ledger:\\[2pt]
making every claim in a simulation pipeline falsifiable\\[8pt]
\large --- demonstrated on a seven-layer derivation pipeline\\
\large for the critical transverse-field Ising chain}

\author{%
  \normalsize JianZhong Yan\\
  \small \textit{(independent researcher)}\\
  \small \texttt{<450353251@qq.com>}
}
\date{\today}
...
\begin{abstract}
\noindent
Multi-stage scientific pipelines produce, at every stage, a derived quantity that
downstream stages consume. Whether a stage ``got it right'' is, in practice,
usually guarded by positivity, finiteness, or non-emptiness checks --- checks that
\emph{do not fail when the conclusion is false}. We state an operational criterion
for a genuine \emph{negative control}: \textbf{it must be able to fail when the
conclusion does not hold}. Applying it explicitly excludes positivity / finiteness /
non-emptiness checks, and sorts the surviving controls into four types
(repulsion, null-model, two-path, upper-bound). The ledger built from them carries
its own \emph{completeness guard}: every guard name it cites must exist in the
runtime registry, and each of the seven layers must appear exactly once --- so the
ledger cannot be written as a brochure. Checks whose guarded proposition is
\emph{measurably false} are registered as \emph{diagnostics} and are
\textbf{excluded from the pass-rate denominator}; otherwise one is grading oneself
on the easy questions.

We demonstrate the method on a seven-layer derivation pipeline whose validation
substrate is the critical transverse-field Ising chain (exact diagonalisation plus
tensor networks). \textbf{Results}: 54 of 54 scored assertions hold, 18 of 18
metrics meet target, and 12 negative controls cover 6 of the 7 layers; the
...
every} $L$ (relative residual $1.81\times10^{-6}$ at $L{=}16$; residual-inverted
power $p=4.00$). A low-energy spectral ratio supplies a second ruler for the
central charge that is \emph{independent of the entanglement-spectrum family}
($0.212\%$ deviation, versus $7.97\%$ for the Rényi family). We report three
\emph{negative} results: on one layer no control can be constructed at all (no
causal path to any downstream consumer --- the cause is given); one layer's
``geometry'' readout \emph{does not depend on the fitted state} (it is an analytic
property of the binary network wiring, and therefore must not be read as an
\emph{emergent} geometry); and two self-contradictory reporting conventions were
corrected.
...
\begin{enumerate}[leftmargin=1.4em,itemsep=2pt]
  ...
  \item \textbf{A ledger with a completeness guard} (Sec.~\ref{sec:ledger}) that
        ...
        than trusting the ledger's own self-report.
...
\begin{itemize}[leftmargin=1.4em,itemsep=2pt]
  ...
  \item We do \textbf{not} claim that any holographic bridge was established
        between layers 4 and 5 (Sec.~\ref{sec:l4l5}).
...
\textbf{tensor-network library constraint}: the MERA construction used requires $L$
...
The substrate is pinned by a comparison to a formula with \emph{no free
parameters}:
\begin{equation}
...
forman value is derived from it, the control family is constructed on it, and the
result is written into the persisted record.
...
construction}, which hard-codes its own bond dimension. The suspicion that this
constitutes an undeclared coupling was raised, and then falsified by the cheapest
available test (Sec.~\ref{sec:l5geometry}); the falsification is what pins the
conclusion down.

\subsection{The ``geometry'' readout is an analytic property of the wiring}
\label{sec:l5geometry}

Layer 5's bulk-geometry readout is computed on a MERA tensor-network graph, and its
presentation invited the reading ``an emergent geometry''. Three measurements show
...
\noindent The comparison machinery is not vacuously true: changing $L$ \emph{does}
change the edge set ($L{=}16$: $|E| = 43$; $L{=}32$: $|E| = 91$), and the tensor
count equals $2L-2$ exactly ($30$ and $62$ respectively). Both the seed and
bond-dimension invariances are now computed \emph{at pipeline runtime} and
interpolated into the guard text, rather than transcribed as constants.

\medskip
\noindent\textbf{This does not make the readout meaningless.} In a MERA the geometry
\emph{is} encoded in the wiring; that is a normal property of the architecture. What
it does is tighten the wording: the readout is a property of the
\textbf{binary-layout wiring}, and the upstream claim must be re-worded accordingly.
Reported values: $|V| = 62$, $|E| = 91$, $\bar d = 2.935$, \textbf{triangles $= 0$},
Forman mean $-2.242$, Ollivier mean $-0.3297$, percentiles $33\%/33\%$.
...
\textbf{Entry point.} A single command, no arguments:

\begin{quote}
\code{python spiral\_model\_v15.py}
\end{quote}

\noindent On a single laptop this takes on the order of $20$ minutes. The run writes
\code{spiral\_v15\_metrics.png} (a 20-panel metric report) and
\code{result/\_v15\_data.json}, and prints the guard ledger to standard output.

\textbf{Expected output.} The following lines are what a correct run prints. They
...
artefacts & \code{spiral\_v15\_metrics.png}, \code{result/\_v15\_data.json} \\
...
\medskip
\noindent\textbf{Falsifiability of the method itself.} The ledger's completeness
guard fails if a cited guard does not exist in the runtime registry or if a layer is
missing. The coverage guard fails if coverage changes. Both are checks on the
accounting, which is precisely their scope: they make the ledger hard to fake, not
the physics true.
